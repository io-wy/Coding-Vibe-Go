...
func main() {
	if len(os.Args) < 4 || os.Args[1] != "--action" {
		...
		fmt.Fprintf(os.Stderr, "  go run scripts/verify-action.go --action \"import internal/core from internal/handler\" github.com/example/proj\n")
		os.Exit(1)
	}
...
func verifyCreateFile(action string, cfg *Config) Result {
	...
	// e.g. "create file internal/types/user.go" or "add file internal/handler/auth.go"
	var filePath string
