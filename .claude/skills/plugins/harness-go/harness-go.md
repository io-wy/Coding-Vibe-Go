...
# 验证 import 方向
go run .claude/skills/harness-go/scripts/verify-action.go --action "import internal/config from internal/types" github.com/example/proj
```
...
LAYER VIOLATION: internal/types/user.go
  internal/types (types, Layer 0) imports internal/config (config, Layer 2)
